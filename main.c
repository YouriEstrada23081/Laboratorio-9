...
int main(void)
{
  ...
  while (1)
  {
	if(archivo==0 && estadopasado==0){
		  ...
		  uint8_t Test[]="Si deseas ver alguno de estos manda la letra correspondiente en miniscula :) \r\n";
		  	  			  HAL_UART_Transmit(&huart2,Test,sizeof(Test),1000);
		  uint8_t Test3[]="En caso quieras desconectar tu tarjeta sd manda una d :) \r\n";
	...
	if(archivo==3){
		  fres = f_open(&fil, "Carita.txt", FA_READ);
		 			if (fres == FR_OK) {
		 		  	transmit_uart("File opened for reading.\n");
		 		  	} else if (fres != FR_OK) {
		 		  	 transmit_uart("File was not opened for reading!\n");
